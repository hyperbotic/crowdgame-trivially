...
class Position
  ...
  @copy: (position)->
    ...
    new Position(position.top, position.left)
  ...
  # ----------------------------------------------------------------------------------------------------------------
  near: (position, within)->

    isNear = false

    isNear = Math.abs(@top - position.top) <= within
    isNear = isNear and Math.abs(@left - position.left) <= within
...
#    @options.borderColor = Hy.UI.Colors.white
  ...
  mapUIPropertyIntoView: (propName, parentView)->
    ...
    if dimensionToMap? and dimensionToIgnore?
      point = {}
      point[dimensionToMap] = this.getUIProperty(propName)
      point[dimensionToIgnore] = 0

      mappedPoint = this.convertPointToView(point, parentView)
...
#    this.applyToChildren("initialize")
  ...
  removeChildren: ()->
    ...
    while _.size(@children) isnt 0
      this.removeChild_(_.first(@children))
...
class ButtonProxy extends SystemButtonProxy
  ...
  processOptions: (options)->
    ...
    if not options.backgroundImage?
      if (options._imageSelected? and options._imageNotSelected?) 
        defaultOptions._imageSelected = options._imageSelected
        defaultOptions._imageNotSelected = options._imageNotSelected      
      else
        image = switch options._style
          when "check"
            "circle-blue-check"
          when "plain", "plainOnDarkBackground"
            "circle-blue"
          else
            "circle-blue"

        defaultOptions._imageSelected = "assets/icons/" + image + ".png"

        image = switch options._style
          when "check"
            "circle-black-blue-rim"
          when "plainOnDarkBackground"
            "circle-black-blue-rim"
          when "plain"
            "circle-black"
          else
        ...
        defaultOptions._imageNotSelected = "assets/icons/" + image + ".png"

      defaultOptions.backgroundSelectedImage = defaultOptions._imageSelected
      defaultOptions.backgroundImage = defaultOptions._imageNotSelected
  ...
  setEmphasisUI: (emphasis)->
    ...
    if prop? and (value = this.getUIProperty(prop))?
      this.setUIProperty("backgroundImage", value)
    else
      super
...
class ButtonProxyWithState extends ButtonProxy
  ...
  setSelected: (isSelected)->
    ...
    this.setEmphasisUI(if @selected then "selected" else "notSelected")
...
class ImageViewProxy extends ViewProxy
  ...
  setEmphasisUI: (emphasis)->
    ...
    if prop? and (value = this.getUIProperty(prop))?
      this.setUIProperty("image", value)
    else
      super
...
class OptionsListBase extends ViewProxy
  ...
  kDefaultTextColor = Hy.UI.Colors.black
  kDefaultLabelHeight = 30

  # ----------------------------------------------------------------------------------------------------------------
  constructor: (options, @labelOptions, @choiceOptions, @stackingOptions)->
...
#      borderWidth: 1
  ...
  addLabel: ()->
    ...
    @label = new LabelProxy(Hy.UI.ViewProxy.mergeOptions(defaultOptions, @labelOptions))
...
class OptionsList extends OptionsListBase
  ...
  constructor: (options, labelOptions, choiceOptions, stackingOptions)->
    ...
    super options, labelOptions, choiceOptions, stackingOptions
...
#
  addSelectorControl: (selectorText)->
    ...
    options = 
      ...
      height: @choiceOptions.height
      _style: @choiceOptions._style
      font: Hy.UI.Fonts.mergeFonts(Hy.UI.Fonts.specSmallNormal, {fontSize: 20})
      color: Hy.UI.Colors.white
...
class ScrollView extends ViewProxy
  ...
  # ----------------------------------------------------------------------------------------------------------------
  constructor: (options, @childViewOptions, fnClick = null, fnTouchAndHold = null, @fnScroll = null)->
...
#      view.setUIProperty("backgroundColor", if flag then Hy.UI.Colors.gray else null)
  ...
  computeNumVisibleViews: ()->
    ...
    if not (dividerThickness = this.getUIProperty("_dividerImageThickness"))?
      dividerThickness = 0
  ...
  initViews: ()->
    ...
    if (dividerImage = this.getUIProperty("_dividerImage"))?
      dividerOptions = 
        image: dividerImage
#        borderColor: Hy.UI.Colors.white
      dividerOptions[propThickness] = this.getUIProperty("_dividerImageThickness")
    else
      dividerOptions = null
    ...
    for child in this.getUIProperty("_fnViews")()
      ...
      if addDivider and dividerOptions?
        this.addChild(new ImageViewProxy(dividerOptions))
...
class ScrollOptions extends ViewProxy
  ...
  # ----------------------------------------------------------------------------------------------------------------
  constructor: (options = {}, @labelOptions = {}, @scrollOptions = {}, @childViewOptions = {}, arrowsOptions = null, @fnClicked, @fnTouchAndHold)->
...
#      borderColor: Hy.UI.Colors.blue
    ...
    scrollViewContainer.addChild(@scrollView = new ScrollView(@scrollOptions, @childViewOptions, @fnClicked, @fnTouchAndHold, ((view)=>this.scrolled())), true)
