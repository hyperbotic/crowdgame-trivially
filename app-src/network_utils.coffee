...
#    Hy.Trace.debug "HTTPEvent::getHTTPClient (client=#{gHTTPClient})"
    ...
    if NetworkService.isOnline()
      if not gHTTPClient?
        try
          ...
          gHTTPClient = null
          new Hy.Utils.ErrorMessage "error", "NetworkService::getHTTPClient", "Failed to create HTTP Client"
    else
  ...
  send: ()->
    ...
    if httpClient?
      ...
      httpClient.open 'GET', this.getURL() #, false # 2.6.2 https://jira.appcelerator.org/browse/TC-4836

      httpClient.setRequestHeader 'User-Agent', this.getUAS()

      httpClient.timeout = Hy.Config.NetworkService.kDefaultEventTimeout
...
class NetworkService
  ...
  # ----------------------------------------------------------------------------------------------------------------
  @isOnlineWifi: ()->
    Ti.Network.networkTypeName is "WIFI"
  ...
  @setNetworkChanged: ()->
    ...
    NetworkService.get()?.setNetworkChanged()

  # ---------------------------------------------------------------------------------------------------------------- 
  @getAddressEncoding: ()->

    NetworkService.get()?.getAddressEncoding()

  # ---------------------------------------------------------------------------------------------------------------- 
  @setConsoleHTTPPort: (consoleHTTPPort)->

    NetworkService.get()?.setConsoleHTTPPort(consoleHTTPPort)

  # ---------------------------------------------------------------------------------------------------------------- 
  @setBonjourPublishInfo: (serviceStatus)->

    NetworkService.get()?.setBonjourPublishInfo(serviceStatus)
  ...
  setConsoleHTTPPort: (consoleHTTPPort)->
    ...
    Hy.Trace.info "NetworkService::setConsoleHTTPPort (port=#{consoleHTTPPort})"

    if @consoleHTTPPort isnt consoleHTTPPort
      @consoleHTTPPort = consoleHTTPPort

      this.setNetworkChanged("Wifi")
  ...
  # ---------------------------------------------------------------------------------------------------------------- 
  bonjourPublishInfoHasChanged: (newBonjourPublishInfo)->

    # First test:  true if one is null
    (newBonjourPublishInfo isnt @bonjourPublishInfo) or not _.isEqual(newBonjourPublishInfo, @bonjourPublishInfo)
      
  # ---------------------------------------------------------------------------------------------------------------- 
  setBonjourPublishInfo: (serviceStatus)->

    if (b = serviceStatus.bonjourPublish)?

      Hy.Trace.info "NetworkService::setBonjourPublishInfo (#{b.status})"

      if this.bonjourPublishInfoHasChanged(b)
        switch b.status
          when "bonjourPublishSuccess"
            @bonjourPublishInfo = b
          when "bonjourPublishFailure"
            @bonjourPublishInfo = null

        this.setNetworkChanged("Bonjour")

    this

  # ---------------------------------------------------------------------------------------------------------------- 
  convertBonjourHostname: (hostname)->
    ...
    newHostname = newHostname.replace(/[ ]/gi, "-")

    newHostname = newHostname.replace(/[^a-z0-9-]/gi, "")
  ...
  getBonjourURL: ()->
    ...
    url = if (b = @bonjourPublishInfo)? and (port = this.getConsoleHTTPPort())?
      # Lowercase "name", and change spaces to dashes, per
      # http://docs.info.apple.com/article.html?path=Mac/10.4/en/mh1670.html
      "http://#{this.convertBonjourHostname(b.name)}.#{b.domain}:#{port}"
    else
      null
  ...
  getIPURL: ()->
    ...
    url = if (ip = NetworkService.getIP())? and (port = this.getConsoleHTTPPort())?
      "http://#{ip}:#{port}"
    else
      null
    ...
    switch reason
      when "Wifi"
        if Hy.Network.NetworkService.isOnlineWifi()
          s += "WiFi on"
          if (encoding = this.getAddressEncoding())?
            s+= " IP=#{Hy.Network.NetworkService.getIP()} Encoding=#{encoding}"
        else
          s += "WiFi off"
        ...
        if encoding isnt @encoding
          @encoding = encoding
          notify = true
          this.updateRendezvous(true)

      when "Bonjour"
        notify = true

    Hy.Trace.info "NetworkService::setNetworkChanged (Reason = #{reason} #{s} Notify? #{notify})"

    if notify
      NetworkService.notifyObservers (observer)=>observer.obs_networkChanged?(reason)
  ...
  getAddressEncoding: ()->
    ...
    if Hy.Network.NetworkService.isOnlineWifi() and @consoleHTTPPort?
      e = new Hy.Network.IPAddressEncoder()
      encoding = e.encode(Hy.Network.NetworkService.getIP(), @consoleHTTPPort)

    encoding

  # ----------------------------------------------------------------------------------------------------------------
  # Keep our cloud-based rendezvous service appraised of our local IP/Port
  #
  updateRendezvous: (force = false)->

    # To avoid spamming the service
    if not force and @rendezvousEvent? and (@rendezvousEvent.getTimeSinceSent() <= Hy.Config.Rendezvous.MinConsoleUpdateInterval)
      Hy.Trace.debug "ConsoleApp::updateRendezvous (not updating - waiting for min interval)"
      return this

    fn = (e, status)=>Hy.Trace.debug("NetworkService::updateRendezvous (callback status=#{status} #{e.responseText})")

    @rendezvousEvent = new Hy.Network.HTTPEvent()
    @rendezvousEvent.setFnPost(fn)

    url = Hy.Config.Rendezvous.API
    url += "/consoles?register"
    url += "&display=#{escape(Ti.Platform.username)}"
    url += "&iip=#{Hy.Network.NetworkService.getIP()}"
    url += "&port=#{@consoleHTTPPort}"

    @rendezvousEvent.setURL(url)

    if Hy.Network.NetworkService.isOnlineWifi() and @consoleHTTPPort?
      if not @rendezvousEvent.enqueue()
        Hy.Trace.debug "NetworkService::updateRendezvous (COULD NOT ENQUEUE EVENT)"
