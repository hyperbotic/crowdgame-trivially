...
#    Hy.Trace.debug "Player::constructor(label => #{@label})"
    ...
    @avatar = Hy.Avatars.AvatarSet.getCurrent().assignAvatar()
...
#    Hy.Trace.debug "Player::@destroy(tag => #{player.tag})"
  ...
  # ----------------------------------------------------------------------------------------------------------------
  dumpStr: ()->
    "#{this.constructor.name}: index=#{@index} #{@name} avatar=#{@avatar.getDumpStr()} label=#{@label} status=#{if @status==Player.kStatusActive then "Active" else "Inactive"}"
  ...
  # ----------------------------------------------------------------------------------------------------------------
  @removePlayer: (player)->
    Hy.Avatars.AvatarSet.getCurrent().unassignAvatar(player.getAvatar())
  ...
  setName: (requestedName, systemAssigned = false) ->
    ...
    fnShorten = (n)=>
      n?.slice(0, kMaxPlayerNameLength)
    ...
    if not errorMessage?
      @name = givenName
      this.getAvatar().setPlayerName(@name)
      null
