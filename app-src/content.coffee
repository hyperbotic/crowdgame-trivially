...
class ContentPack
  ...
  @kRequiredFields = ["kind", "filename", "directory", "contentFileKind", "productID", "version", "method", "numRecords", "longDescription", "displayName", "difficulty", "iconSpec"]
  ...
  # (due to loging in "initPurchaseState")
  #
...
class ContentFile
  ...
  # ----------------------------------------------------------------------------------------------------------------
  getTriviaCountStartLineNum: ()-> 1

  # ----------------------------------------------------------------------------------------------------------------
  checkTriviaContent: (contentPack, content = [])->

    lineNum = this.getTriviaCountStartLineNum()
...
class ContentFileWithHeader extends ContentFile
  ...
  # ----------------------------------------------------------------------------------------------------------------
  # We add one to account for the blank line separator
  #
  getTriviaCountStartLineNum: ()-> @numHeaderRecords + 1
  ...
  getHeader: (content = [])->
    ...
    if (header = this.checkHeaderPropConstraints(content))?
      if not @numHeaderRecords?
        header = null
        new Hy.Utils.ErrorMessage("fatal", "ContentFileWithHeader::getHeader", "Internal Error: \"numHeaderRecords\" is null") #will display popup dialog

    header

  # ----------------------------------------------------------------------------------------------------------------
  checkHeaderPropConstraints: (content)->
    ...
    for c in content
      ...
      switch name
        ...
        when "numRecords"          
          if (header.numRecords = value) < (min = Hy.Config.Content.kThirdPartyContentPackMinNumRecords)
    ...
    for propSpec in kRequiredHeaderProperties
      if not _.detect(content, (c)=>c.name is propSpec.displayName)?
  ...
  checkHeaderProp: (name, value, lineNum, header, defaultProps)=>
    ...
    switch name
      ...
      when "description" # This will be mapped into "longDescription"
        ...
        value = fnIllegalCharsCheck(name, value)
        header.longDescription = value

      when "difficulty"
        defaultValue = "easy"
        value = fnStringEmptyCheck(name, value, "defaulted to \"#{defaultValue}\"", defaultValue)

        header.difficulty = switch value.toLowerCase()
          when "easy"
            1
          when "medium"
            2
        ...
        if value.length > Hy.Config.Content.kContentPackMaxIconSpecLength
          ...
          if not warn?
            if not ContentIcon.findByIconSpec(value.toLowerCase())?
              ...
              value = defaultValue
        header.iconSpec = value.toLowerCase()
      else
        found = false
...
class ExcelContentFile extends ContentFileWithHeader
  ...
  getTriviaContent: (contentPack)->
    ...
    if (lines = this.tokenize())?
      ...
      for i in [@numHeaderRecords..lines.length-1] # Skip the header
  ...
  getHeader: (content = [])->
    ...
    fnMessage = (kind, s)=>
      ContentManager.addMessage(kind, numLines, s)
      if kind is "error"
        error = true
      null
          ...
          if prop is this.getFileTag().slice().toLowerCase()
            ...
            switch mode
              when "header"
                ...
                if numLines > (max = Hy.Config.Content.kContentPackWithHeaderMaxNumHeaderProps)
                  fnMessage("error", "Too many settings (max is #{max})")
                  break
                else
                  # Have we seen this property before?
                  if _.size(_.select(content, (r)=>r.name is prop)) > 0
                    fnMessage("warning", "Ignoring duplicate setting \"#{propDisplay}\"")
                  else
                    content.push {name: prop, value: value}
    ...
    r = if error
      ...
      @numHeaderRecords = numHeaderLines # Include separator
      content.push {name: "numRecords", value: lines.length - @numHeaderRecords}
      super content
...
class ThirdPartyContentManifest extends DirectoryDerivedContentManifest
  ...
  refreshContentInitialize: (eventSpecs)->
    ...
    downloadManager = new Hy.Network.DownloadManager(this, 
                                                     ...
                                                     ((cm, status)=>cm.refreshContentDone(status)),
  ...
  refreshContentDownloaded: (event, eventSpec)->
    ...
    if not eventSpec.isCoerced? 
      ...
      if coercedURL?
        ...
        eventSpec.coercedURL = coercedURL # Will be the signal that we are still chasing this content
  ...
  refreshContentDone: (status)=>
    ...
    # Should be exactly one element in 'status', but this bit of code is arranged to handle more than one
    for s in status
      Hy.Trace.debug "ThirdPartyContentManifest::refreshContentDone (status=#{s.object.display} #{s.status})"

      # Set up for warn/error messages
      ContentManager.clearMessages()

      if s.status
        if s.object.coercedURL? # Means we are still chasing this one... skip
          moreToCome = true
        else
          newContentPack = null

          if (newContentPackSpec = this.refreshContentProcess(s.object))?
            for newContentPack in this.processContentPacks([newContentPackSpec])
              @refreshState.contentPacks.push newContentPack

              newContentPack.load()

              # Make it appear selected
              newContentPack.setSelected(true)
...
#        if not s.object.rawText?
#          ContentManager.addMessage("error", null, "Download Failed. Check URL, and use Google \"publish\" option & don't select \"Require viewers to sign in")

      @refreshState.messages = this.refreshContentProcessMessages(@refreshState.messages, s.object, newContentPackSpec, newContentPack)

    @refreshState.numSuccess = @refreshState.numAdded + @refreshState.numUpdated

    ContentManager.clearMessages()

    if moreToCome? and moreToCome
      null # Just exit and wait for the next invocation
    else
      # Report on how things went. This code assumes that we're operating on one content pack at a time
      ...
      if @fnRefreshDone?
        ...
        contentPack = if successFlag then _.first(@refreshState.contentPacks) else ""
  ...
  refreshContentProcess: (eventSpec)->
    ...
    contentPackInfo = switch eventSpec.operation
      ...
      when "refresh"
        ...
        Hy.Trace.debug "ThirdPartyContentManifest::refreshContentProcess (NO CONTACT PACK INFO)"
        null
    ...
    if contentPackInfo?
      ...
      if eventSpec.rawText.length > (maxSize = contentPackInfo.kind.getMaxContentFileSize())
        ContentManager.addMessage("error", null, "Content must be smaller than #{(maxSize/1024).toPrecision(1)} KB)")
      else
        ...
        if contentPackInfo.kind.checkTypeViaPeek(eventSpec.rawText)
          ...
          if (newContentFile = contentPackInfo.kind.writeNewContentFile(contentPackInfo.directory, contentPackInfo.productID, contentPackInfo.newVersion, eventSpec.rawText))?
            ...
            if (newContentPackSpec = this.deriveContentPackSpec(newContentFile.getFilename()))?
              ...
              if newContentFile? # Clean up
                ContentFile.delete(newContentFile)
                newContentFile = null
          ...
          report = "First cell of first row must contain \"##Trivia\""
  ...
  # ----------------------------------------------------------------------------------------------------------------
  refreshContentProcessMessages: (messages, eventSpec, newContentPackSpec, newContentPack)->
  ...
  # ----------------------------------------------------------------------------------------------------------------
  refreshContentGetContext: (eventSpec, newContentPackSpec, newContentPack)->

    # Figure out the best context in which to deliver any messages
    maxURLDisplayLength = 60
    if (displayURL = eventSpec.URL).length > maxURLDisplayLength
      displayURL = "#{displayURL.substr(0, 15)}...#{displayURL.substr(eventSpec.URL.length-30)}"

    maxLen = 10

    fnDisplay = (text, maxLen)=>
      "\"#{text.substr(0, maxLen) + if text.length > maxLen then "..." else ""}\""
      
    context = if eventSpec.contentPackInfo? and (displayName = eventSpec.contentPackInfo.displayName)?
      fnDisplay(displayName, maxLen)
    else if newContentPackSpec? and (displayName = newContentPackSpec.displayName)?
      fnDisplay(displayName, maxLen)
    else if newContentPackSpec? and (description = newContentPackSpec.description)?
      fnDisplay(description, maxLen)
    else
      displayURL
...
class ContentManager
  ...
  # ----------------------------------------------------------------------------------------------------------------
  userCreatedContentAddRequested: ()->
    ...
    fnDone = (context, status, navSpec, changes)=>
      if status
        ...
        navSpec._buttonSpecs = [
          {_value: "show me how to get started!", _dismiss: "_root", _fnCallback: (event, view)=>Hy.UI.Application.get().showUserCreatedContentInfoPage()},
...
class UCC_Activity extends ContentManagerActivity
  ...
  doAdd: ()->
    ...
    if Ti.UI.Clipboard.hasText() and (url = Ti.UI.Clipboard.getText())?
      ...
      navSpec = 
        _title: title
        _explain: "Clipboard is empty!\nPlease copy to the clipboard the URL of the topic you would like to import, and try again"
        _buttonSpecs: [
  ...
  doUpsell: (context = {}, topLevelTransaction = true)->
    ...
    fnShowInfoPage = (report)=>
      Hy.UI.Application.get().showUserCreatedContentInfoPage()
      null
